...
public class DataFetcherDefinitions {
    ...
    final DataFetcher<Mono<Optional<Car>>> carFetcher = (environment) -> {
    ...
    final DataFetcher<Mono<Driver>> driverFetcher = (environment) -> {

        Car car = environment.getSource();
        return driverFacade.getDriver(car.getDriverId());
    };

    final DataFetcher<Mono<List<DrivingFine>>> penaltiesFetcher = (environment) -> {
    ...
    final DataFetcher<Mono<Integer>> activePenaltyPointsFetcher = (environment) -> {
        Driver driver = environment.getSource();
        DataLoader<String, List<DrivingFine>> dataLoader = environment.getDataLoader("drivingFines");
    ...
    final DataFetcher<Mono<Optional<Car>>> drivingFineCarFetcher = (environment) -> {
